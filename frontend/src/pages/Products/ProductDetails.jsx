...
const ProductDetails = () => {
    ...
    const [createReview, { isLoading: loadingProductReview }] = useAddReviewMutation()
    ...
    return (
        <>
            <div>
                <Link to='/' className="text-white font-semibold hover:underline ml-[5rem]">Go Back</Link>
            </div>
            ...
            {isLoading ? (<Loader />) : error ? (<Message variant={'danger'}>{error?.data?.message || error.message}</Message>) : (
                <>
                    <div className="flex flex-wrap relative items-between mt-[2rem] ml-[5rem]">
                        <div>
                            <img src={product.image} alt={product.name} className="w-full xl:w-[40rem] lg:w-[35rem] md:w-[25rem] sm:w-[25rem] mr-[2rem]" />
                        ...
                        <div className="flex flex-col justify-between">
                            ...
                            <p className="my-4 xl:w-[25rem] lg:w-[20rem] md:w-[10rem] text-[#B0B0B0]">{product.description}</p>
                            ...
                            <div className="flex justify-between flex-wrap">
                                ...
                                {product.countInStock > 0 && (
                                    <div>
                                        <select value={quantity} onChange={(e) => setQuantity(e.target.value)} className="p-2 w-[6rem] rounded-lg text-black">
